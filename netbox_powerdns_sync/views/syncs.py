...
class SyncJobsView(ContentTypePermissionRequiredMixin, View):
    ...
    def get(self, request):
        query = Q(app_label="netbox_powerdns_sync", model="zone") | Q(
        ...
        jobs_table = SyncJobTable(data=jobs, orderable=False, user=request.user)
        jobs_table.configure(request)

        return render(
            request,
            "netbox_powerdns_sync/syncs.html",
            {
                "table": jobs_table,
                "tab": "jobs",
            },
...
class SyncResultView(ContentTypePermissionRequiredMixin, View):
    ...
    def get(self, request, job_pk):
        ...
        return render(
            request,
            "netbox_powerdns_sync/sync_result.html",
            {
                # "script": script,
                "job": job,
            },
...
class SyncScheduleView(View):
    def get(self, request):
        ...
        jobs_table = SyncJobTable(
            data=scheduled_jobs, orderable=False, user=request.user
        )
        ...
        return render(
            request,
            "netbox_powerdns_sync/sync_schedule.html",
            {
